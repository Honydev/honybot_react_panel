...
class App extends Component {
  ...
  constructor (props) {
    ...
    this.state = {
      ...
      currencyName: window.honybot.currencyName,
      loading: {
        active: false,
        percent: 20
      },
  ...
  getAlertbar () {
    return <div className='alert alert-warning text-center info-bar' role='alert' style={{'borderRadius': 0, 'backgroundColor': '#677bc4', 'color': '#fff', 'boxShadow': 'none'}}>
      <span>
        <i className='fab fa-github' style={{'fontSize': '17px'}} /> <strong>We released Honybot as open source on <a href='https://github.com/Honydev/honybot' style={{'color': '#2c343f'}} target='_blank' rel='noopener noreferrer'>GithHub <i className='fas fa-external-link-alt' /></a></strong>
      </span>
    </div>
  }
