...
class ControlPanel extends Component {
  constructor (props) {
    super(props)
    this.state = {
      botRunning: window.honybot.botRunning,
      starting: false,
      restarting: false,
      stopping: false,
      setCommunity: window.honybot.community,
      toUser: {
        name: '',
        amount: 0
      },
  ...
  handleStartResponse (response) {
    if (response.type === 'success') {
      ...
      window.honybot.botRunning = true
      toast.success(response.text)
    } else {
      toast.error(response.text)
    }
  ...
  handleStopResponse (response) {
    if (response.type === 'success') {
      ...
      window.honybot.botRunning = false
      toast.success(response.text)
    } else {
      toast.error(response.text)
    }
        ...
        <div className='row'>
          <div className='col-md-12'>
            <label className='css-input switch switch-success'>
              <input type='checkbox' id='community' checked={this.state.setCommunity} onChange={this.toggleCommunity} /><span /> Automatically set community to Honybot when bot starts
            </label>
          </div>
        </div>
      </div>
